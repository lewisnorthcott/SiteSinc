...
private var toolbarButtons: some ToolbarContent {
    ToolbarItemGroup(placement: .topBarTrailing) {
        ...
        .disabled(isMarkupUIActive || currentPdfFile == nil || isDownloadingForShare)
                ...
                DragGesture(minimumDistance: 10)
                    .onChanged { value in
                        ...
                        if swipeStartPoint == nil {
                            swipeStartPoint = value.startLocation
                            swipeStartTime = Date()
                        }
                    }
                    .onEnded { value in
                        ...
                        if horizontalSwipe && beganAtEdge {
                            if value.translation.width < -swipeThreshold {
                                // Swipe left from right edge - next drawing
                                if drawingIndex < drawingsCount - 1 {
                                    withAnimation(.easeInOut) { drawingIndex += 1 }
                                    UIImpactFeedbackGenerator(style: .light).impactOccurred()
                                }
                            } else if value.translation.width > swipeThreshold {
                                // Swipe right from left edge - previous drawing
                                if drawingIndex > 0 {
                                    withAnimation(.easeInOut) { drawingIndex -= 1 }
                                    UIImpactFeedbackGenerator(style: .light).impactOccurred()
                                }
